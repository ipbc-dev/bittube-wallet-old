...
int main(int argc, char* argv[]) {
	...
	if (!QDir().exists(dataDirPath)) {
		...
		if (dataDir.exists("ipbcwallet.lock")) {
			QLockFile oldLockFile(dataDir.absoluteFilePath("ipbcwallet.lock"));
		...
		if (dataDir.exists("ipbcwallet.wallet")) {
			QString files[] = {"ipbcwallet.cfg", "ipbcwallet.log", "ipbcwallet.wallet", "ipbcwallet.wallet.backup", "ipbcwallet.lock"};
			for (auto f : files) QFile().rename(dataDir.absoluteFilePath(f), dataDir.absoluteFilePath(f).replace("ipbc", "bittube"));
		}
	}
